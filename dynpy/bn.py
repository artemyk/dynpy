...
def tuple2int(b):
    """Helper function which converts a binary representation (e.g., ``[1,0,1]``) into an integer
    """
    return int("".join(map(str, map(int, b))), 2)


def int2tuple(i, num_places):
    """Helper function which converts an integer into a binary representation (in the form of a numpy array of 0s and 1s).
    The binary representation will be `num_places` long, with extra places padded with 0s.
    """
...
class BooleanNetwork(dynsys.DiscreteStateSystemBase):
    ...
    The network is specified using the `rules` parameter, which is a list with one element 
    for each Boolean variable.  Each of these elements is itself a list, containing 3 parts:
    ...
    The dynamical update rules can be passed in in two ways.  If the `mode` parameter is equal to 
    `'TRUTHTABLES'` (the default), then it should be a truth table, represented as a list of 0s and 1s.
    The first element of this list corresponds to the desired output when all the inputs are on 
    (i.e., are all 1s), while the last element of this list corresopnds to the desired output when 
    all the inputs are off.  For example:
    ...
    def _getVarNextStateTT(self, varIndex, inputs):
        """ Execute update rule when network is specified using truthtables.  Repointed in constructor. """
        return self.rules[varIndex][2][-1 - tuple2int(inputs)]

    def _getVarNextStateFuncs(self, varIndex, inputs):
        """ Execute update rule when network is specified using functions. Repointed in constructor. """
        return self.rules[varIndex][2](*inputs)

    @caching.cached_data_prop
    def ndx2stateMx(self):
        """
        ``(num_states, num_vars)``-shaped matrix which maps from state indexes to their representations
        in terms of the activations of the Boolean variables.
        """
        num_states = 2**self.num_vars
        ndx2stateMx = np.fromiter( itertools.chain(* (int2tuple(s, self.num_vars) for s in range(num_states)) ) , dtype='u1')
    ...
    def trans(self):
        ...
        scipy.sparse matrix (for sparse representations)
        """

        if self.num_vars > 20:
            raise Exception('Computing transition matrix for a %d-variable BN will take too long' % self.num_vars)
    ...
    def _inputs(self):
        """
        Remaps inputs from being specified by variable names to being specified by variable indexes.
        Makes update functions run faster.
        """
    ...
    def printAttractorsAndBasins(self):
        ...
        >>> import dynpy
        >>> bn = dynpy.bn.BooleanNetwork(rules=[ ['a',['a','b'],[1,1,1,0]],['b',['a','b'],[1,0,0,0]]])
    ...
    def checkTransitionMatrix(self, trans):
        """
        Internally used function that checks the integrity/format of the generated transition matrix.
        """
